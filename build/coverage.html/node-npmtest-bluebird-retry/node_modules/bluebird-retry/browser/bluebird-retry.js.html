...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-bluebird-retry/node_modules/bluebird-retry/browser/bluebird-retry.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    <h1 style="font-weight: bold;">
        <a href="https://github.com/npmtest/node-npmtest-bluebird-retry">npmtest-bluebird-retry (v2017.4.26)</a>
    </h1>
    <h1>Code coverage report for <span class="entity">node-npmtest-bluebird-retry/node_modules/bluebird-retry/browser/bluebird-retry.js</span></h1>
    <h2>
        
        Statements: <span class="metric">28.57% <small>(28 / 98)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Branches: <span class="metric">20.27% <small>(15 / 74)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="path"><a href="../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-bluebird-retry/node_modules/bluebird-retry/browser/</a> &#187; bluebird-retry.js</div>
</div>
<div class="body">
...
144
145
146</td><td class="line-coverage"><span class="cline-any cline-yes">3</span>
...
//
<span class="fstat-no" title="function not covered" >function StopError(err) {</span>
<span class="cstat-no" title="statement not covered" >    this.name = 'StopError';</span>
...
<span class="cstat-no" title="statement not covered" >        this.message = err || 'cancelled'</span>
    }
}
...
//
&nbsp;
<span class="fstat-no" title="function not covered" >function retry(func, options) {</span>
<span class="cstat-no" title="statement not covered" >    options = options || {};</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    var interval = typeof options.interval === 'number' ? options.interval : 1000;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    var max_tries, giveup_time;</span>
<span class="cstat-no" title="statement not covered" >    if (typeof(options.max_tries) !== 'undefined') {</span>
<span class="cstat-no" title="statement not covered" >        max_tries = options.max_tries;</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (options.timeout) {</span>
<span class="cstat-no" title="statement not covered" >        giveup_time = new Date().getTime() + options.timeout;</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!max_tries &amp;&amp; !giveup_time) {</span>
<span class="cstat-no" title="statement not covered" >        max_tries = 5;</span>
    }
&nbsp;
...
&nbsp;
    ...
    // that should retry.
<span class="cstat-no" title="statement not covered" >    var predicate = options.predicate || <span class="fstat-no" title="function not covered" >function(err) {</span> <span class="cstat-no" title="statement not covered" >return true; </span>}</span>
<span class="cstat-no" title="statement not covered" >    var stopped = false;</span>
&nbsp;
<span class="fstat-no" title="function not covered" >    function try_once() {</span>
<span class="cstat-no" title="statement not covered" >        var tryStart = new Date().getTime();</span>
<span class="cstat-no" title="statement not covered" >        return Promise.attempt(<span class="fstat-no" title="function not covered" >function() {</span></span>
<span class="cstat-no" title="statement not covered" >                return func.apply(options.context, options.args);</span>
            })
            .caught(StopError, <span class="fstat-no" title="function not covered" >function(err) {</span>
<span class="cstat-no" title="statement not covered" >                stopped = true;</span>
<span class="cstat-no" title="statement not covered" >                if (err.err instanceof Error) {</span>
<span class="cstat-no" title="statement not covered" >                    return Promise.reject(err.err);</span>
                } else {
<span class="cstat-no" title="statement not covered" >                    return Promise.reject(err);</span>
                }
            })
            .caught(predicate, <span class="fstat-no" title="function not covered" >function(err) {</span>
...
<span class="cstat-no" title="statement not covered" >                if (tries &gt; 1) {</span>
<span class="cstat-no" title="statement not covered" >                    interval = backoff(interval, options);</span>
                }
<span class="cstat-no" title="statement not covered" >                var now = new Date().getTime();</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >                if ((max_tries &amp;&amp; (tries === max_tries) ||</span>
                    (giveup_time &amp;&amp; (now + interval &gt;= giveup_time)))) {
&nbsp;
<span class="cstat-no" title="statement not covered" >                    if (! (err instanceof Error)) {</span>
<span class="cstat-no" title="statement not covered" >                        var failure = err;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >                        if (failure) {</span>
<span class="cstat-no" title="statement not covered" >                          if (typeof failure !== 'string') {</span>
<span class="cstat-no" title="statement not covered" >                            failure = JSON.stringify(failure);</span>
                          }
                        }
&nbsp;
<span class="cstat-no" title="statement not covered" >                        err = new Error('rejected with non-error: ' + failure);</span>
<span class="cstat-no" title="statement not covered" >                        err.failure = failure;</span>
                    } else <span class="cstat-no" title="statement not covered" >if (options.throw_original) {</span>
<span class="cstat-no" title="statement not covered" >                      return Promise.reject(err);</span>
                    }
&nbsp;
<span class="cstat-no" title="statement not covered" >                    var timeout = new Error('operation timed out after ' + (now - start) + ' ms, ' + tries + ' tries with error: ' + err.message);</span>
<span class="cstat-no" title="statement not covered" >                    timeout.failure = err;</span>
<span class="cstat-no" title="statement not covered" >                    timeout.code = 'ETIMEDOUT';</span>
<span class="cstat-no" title="statement not covered" >                    return Promise.reject(timeout);</span>
                } else {
<span class="cstat-no" title="statement not covered" >                    var delay = interval - (now - tryStart);</span>
<span class="cstat-no" title="statement not covered" >                    if (delay &lt;= 0) {</span>
<span class="cstat-no" title="statement not covered" >                        return try_once();</span>
                    } else {
<span class="cstat-no" title="statement not covered" >                        return Promise.delay(delay).then(try_once);</span>
                    }
                }
            });
    }
...
<span class="fstat-no" title="function not covered" >function backoff(interval, options) {</span>
<span class="cstat-no" title="statement not covered" >    if (options.backoff) {</span>
<span class="cstat-no" title="statement not covered" >        interval = interval * options.backoff;</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (options.max_interval) {</span>
<span class="cstat-no" title="statement not covered" >        interval = Math.min(interval, options.max_interval);</span>
    }
&nbsp;
...
<div class="footer">
    <div class="meta">Generated by <a href="https://github.com/kaizhu256/node-utility2" target="_blank">utility2</a> at Thu Apr 27 2017 21:04:27 GMT+0000 (UTC)</div>
</div>
</body>
</html>
